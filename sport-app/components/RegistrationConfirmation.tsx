...
export default function RegistrationConfirmation({
  ...
  const reference = regId ? regId.slice(0, 8).toUpperCase() : "—";
      ...
      <div className="relative z-10 w-full max-w-md mx-4 rounded-2xl border border-white/20 bg-slate-900/40 p-6 sm:p-8 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] backdrop-blur-xl text-white overflow-hidden">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-2 text-xs text-slate-300 max-w-xs leading-relaxed">
            Your team has been submitted for {tournamentName}. We&apos;ve got
            your payment receipt and your details are now awaiting approval.
          </p>
        </div>

        {/* Status pill */}
        <div className="mt-5 flex items-center justify-center gap-2 rounded-xl border border-amber-500/30 bg-amber-950/20 py-2.5 px-4 text-xs font-medium text-amber-300">
        ...
        <div className="mt-4 rounded-xl border border-white/15 bg-slate-950/40 p-4">
          ...
          <p className="mt-2 text-[11px] text-slate-400 leading-relaxed">
            Keep this reference and your receipt safe. We confirm payments within
            24 hours, after which your registration is final.
          </p>
        </div>
        ...
        <div className="mt-4">
          {pdfUrl ? (
            ...
            <div className="flex w-full items-center justify-center gap-2 rounded-md border border-white/15 bg-slate-950/40 py-2.5 px-4 text-xs font-medium text-slate-400">
              ...
              <span>Receipt is being processed</span>
            </div>
          )}
        </div>
        ...
        <div className="mt-4 flex items-start gap-2 rounded-xl border border-white/15 bg-slate-950/40 p-3 text-[11px] text-slate-300">
          ...
          <span>
            Questions about your payment? Message our official WhatsApp line{" "}
            <a
              href={WHATSAPP_LINK}
              target="_blank"
              rel="noopener noreferrer"
