...
export async function submitRegistration(data: RegistrationInput) {
  ...
  if (data.team_logo) {
    ...
    teamLogoUrl = supabase.storage.from("team-logos").getPublicUrl(logoPath)
      .data.publicUrl;
  }
  ...
  for (const player of data.players) {
    ...
    const consentUrl = supabase.storage
      .from("consent-forms")
      .getPublicUrl(consentPath).data.publicUrl;
    const proofOfAgeUrl = supabase.storage
      .from("proof-of-age")
      .getPublicUrl(agePath).data.publicUrl;
...
export async function uploadPaymentReceipt(
  ...
  const receiptUrl = supabase.storage.from("receipts").getPublicUrl(filePath)
    .data.publicUrl;
